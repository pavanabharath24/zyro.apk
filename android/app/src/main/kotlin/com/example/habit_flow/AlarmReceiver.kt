...
class AlarmReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        // RESCHEDULE IF DAILY
        val frequency = intent.getStringExtra("ALARM_FREQUENCY") ?: "once"
        if (frequency == "daily") {
            Log.d("AlarmReceiver", "Rescheduling daily alarm $id")
            rescheduleAlarm(context, id, title, body, isAlarm, audio, vibrate, frequency)
        }
    }

    private fun rescheduleAlarm(context: Context, id: Int, title: String, body: String, isAlarm: Boolean, audio: Boolean, vibrate: Boolean, frequency: String) {
        val alarmManager = context.getSystemService(Context.ALARM_SERVICE) as android.app.AlarmManager
        val nextTime = System.currentTimeMillis() + 24 * 60 * 60 * 1000 // +24 Hours

        val intent = Intent(context, AlarmReceiver::class.java).apply {
            action = MainActivity.ACTION_ALARM_TRIGGER
            putExtra("ALARM_ID", id)
            putExtra("ALARM_TITLE", title)
            putExtra("ALARM_BODY", body)
            ...
            putExtra("ALARM_AUDIO", audio)
            putExtra("ALARM_VIBRATE", vibrate)
            putExtra("ALARM_FREQUENCY", frequency)
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            if (!alarmManager.canScheduleExactAlarms()) {
                Log.w("AlarmReceiver", "Cannot reschedule exact alarm - permission denied. Trying inexact.")
                alarmManager.setExactAndAllowWhileIdle(android.app.AlarmManager.RTC_WAKEUP, nextTime, pendingIntent)
                return
            }
        }

        val alarmClockInfo = android.app.AlarmManager.AlarmClockInfo(nextTime, pendingIntent)
